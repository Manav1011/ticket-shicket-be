# Payment Gateway Phase 4B — Organizer → Customer Transfer

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `is_paid=True` branch in `OrganizerService.create_customer_transfer` so that when an organizer transfers to a customer with `mode=TransferMode.PAID`, a Razorpay payment link is created, the customer is notified, and allocation is deferred to the `order.paid` webhook.
...
**Files modified:**
- `src/apps/organizer/service.py` — implement paid branch in `create_customer_transfer`
...
### Task: Implement paid flow in `OrganizerService.create_customer_transfer`
...
- [ ] Verify OrganizerService uses `self.repository.session`
- [ ] Check `tests/apps/organizer/test_service.py` exists — add test to it
...
Run: `uv run pytest tests/apps/organizer/test_service.py::test_create_customer_transfer_paid_mode -v`
Expected: FAIL — paid mode returns stub `"not_implemented"`

- [ ] **Step 3: Write implementation**

Replace the `if mode == TransferMode.PAID` stub in `OrganizerService.create_customer_transfer` with:
...
Run: `uv run pytest tests/apps/organizer/test_service.py -v`
...
**1. Spec coverage:**
- `OrganizerService.create_customer_transfer` with `mode=TransferMode.PAID` ✅ (spec Section 8.2)
...
- `transfer_type="organizer_to_customer"` in notes ✅
...
**4. Difference from Plan 4A:**
- No `UserRepository` lookup — customer may not have a user account, so `BuyerInfo` is built directly from phone/email
- `transfer_type="organizer_to_customer"` vs `"organizer_to_reseller"`

Plan complete and saved to `docs/superpowers/plans/2026-05-04-payment-gateway-phase4b-organizer-to-customer.md`.